...
# returns list of files with ending, <ending>
def get_all_files(url, ending):
    soup = BeautifulSoup(requests.get(url).text)
    url_list = []
    for a in soup.find_all('a'):
        href = a['href']
        if href.split('.')[-1]==ending:
            url_list.append(files_url + href)
    return url_list

# get data from input <url> and save to <fname> file
...
# extract data from .csv.gz file, add to state's master file
...
if __name__=='__main__':
    ...
    # check input URL is as expected
    # print files_url
    
    # get all file names
    all_files = get_all_files(files_url, ending) #http://lehd.ces.census.gov/data/lodes/LODES7/ny/rac/ny_rac_S000_JT00_2003.csv.gz
